...
export default function SeniorInformationPage() {
  ...
  // 인수를 받는 형태로 수정
  const saveElderData = async (elderlyData: any) => {
    try {
      ...
      if (data.success) {
        alert('어르신 정보가 성공적으로 저장되었습니다.');
      } else {
        alert('어르신 정보를 저장하는 데 실패했습니다.');
      }
    } catch (error) {
      console.error('어르신 정보 저장 중 오류 발생:', error);
  ...
  const handleRegister = async (event: React.FormEvent) => {
    ...
    const newElder = {
      place_name: placeName, // ✅ 센터 정보 포함
      elderly: {
        name,
        birthYear: Number(year), // ✅ `year` 사용 (올바른 변수명 적용)
        gender,
        careLevel, // ✅ `careLevel`로 변수명 맞춤
        weight: Number(weight),
        diseases: diseaseInfo, // ✅ `diseases` 변수명 통일
        dementiaSymptoms: dementiaInfo, // ✅ 배열 타입 유지
        cohabitant,
      },
    };
    ...
    try {
      ...
      const data: { success: boolean; message?: string } = await response.json(); // ✅ 타입 명시
      if (data.success) {
        alert('어르신 정보가 등록되었습니다.');
        router.push('/api/elders');
      } else {
  ...
  const handleNext = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // 데이터 저장
    saveElderData(elderlyData); // 수정된 데이터를 API로 보내는 함수 호출
