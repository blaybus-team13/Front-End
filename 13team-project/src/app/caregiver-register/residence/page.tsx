    ...
    try {
      ...
      const filteredAddresses = data.documents
        .map((item: any) => {
          ...
          return {
            id: Math.random(), // ✅ API 응답에 id가 없을 경우 랜덤 ID 생성
            location: addressParts.slice(0, 3).join(" "), // ✅ 시, 구, 동까지만 포함
            post: item.road_address ? item.road_address.zone_no : "N/A",
          };
        })
        .filter(
          (addr: Address, index: number, self: Address[]) =>
            index === self.findIndex((a) => a.location === addr.location) // ✅ 중복 제거
        );

      console.log("✅ [CLIENT] 필터링된 주소 목록:", filteredAddresses);
      setAddressResults(filteredAddresses);
    } catch (error) {
      console.error("❌ [CLIENT] 주소 검색 오류:", error);
      setAddressResults([]);
    }
