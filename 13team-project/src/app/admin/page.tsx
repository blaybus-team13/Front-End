...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const id = searchParams.get("id");
    if (!id) {
      console.error("관리자 ID가 제공되지 않았습니다.");
      return;
    }
  
    fetch(`/api/admin?id=${encodeURIComponent(id)}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error("관리자 정보를 불러올 수 없습니다.");
        }
        return res.json();
      })
      .then((data) => {
        if (data.success) {
          ...
          console.error("관리자 정보를 불러오는 데 실패했습니다:", data.message);
        }
      })
    ...
    <div className="relative flex flex-col items-center min-h-screen bg-white w-full font-[Pretendard] pb-24">
        ...
        <div className="w-screen h-[115px] bg-[#FF8B14] text-white rounded-b-2xl shadow-md flex items-center justify-between px-6 pt-8 relative">
            ...
            <span className="text-lg lg:text-xl">
                <span className="text-[18px] font-medium">{admin?.position ?? "직책 없음"} </span>
            ...
            {showDropdown && (
                ...
                <button
                    ...
                    onClick={() => {
                    setShowDropdown(false);
                    router.push(`/senior-info/senior-info?id=${encodeURIComponent(id ?? "")}`);
                    }}
                >
                    어르신 정보 등록
                </button>
        ...
        {filteredElders.length === 0 ? (
            ...
            <button
                onClick={() => router.push(`/senior-info/senior-info?id=${encodeURIComponent(id ?? "")}`)}
            ...
            >
                ...
                {filteredElders[currentIndex]?.elderly?.location || "위치 정보 없음"}
                </p>
                <p className="text-lg font-bold mt-1">
                {filteredElders[currentIndex]?.elderly?.name || "이름 없음"}님{" "}
                <span className="text-[#767676] font-medium">
                    | {filteredElders[currentIndex]?.elderly?.gender} {new Date().getFullYear() - (filteredElders[currentIndex]?.elderly?.birthYear || 0)}세, {filteredElders[currentIndex]?.elderly?.careLevel || "등급정보 없음"}
                </span>
                </p>
        ...
        filteredElders.length > 0 && (
            <div className="text-center mt-12">
              <button
                ...
                onClick={() => router.push(`/job-post/basic?elderId=${filteredElders[currentIndex]?.elid}&id=${id}`)}
              >
                +
              </button>
        ...
        <button
            className="text-gray-600 text-[13px] flex flex-col items-center"
            onClick={() => router.push(`/elders?id=${encodeURIComponent(id ?? "")}`)}
        >
