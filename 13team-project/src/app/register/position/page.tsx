...
export default function PositionPage() {
  ...
  const handleNext = () => {
    if (position.trim()) {
      router.push(
        `/register/account?role=${encodeURIComponent(role)}&name=${encodeURIComponent(name)}&phone=${encodeURIComponent(phone)}&position=${encodeURIComponent(position)}`
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 py-4">
      ...
      {/* ✅ role 값을 확인하기 위한 hidden input (디버깅용) */}
