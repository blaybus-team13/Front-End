...
export default function NamePage() {
  ...
  const role = searchParams.get("role") || ""; // ✅ URL에서 role 가져오기
  const [name, setName] = useState<string>("");

  const handleNext = () => {
    if (name.trim()) {
      // ✅ role과 함께 name을 다음 페이지로 넘기기
      router.push(`/register/phone?role=${encodeURIComponent(role)}&name=${encodeURIComponent(name)}`);
    }
  };
